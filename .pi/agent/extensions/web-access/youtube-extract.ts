...
import { formatSeconds, mapFfmpegError, readExecError, trimErrorText } from "./utils.js";
...
export async function extractYouTube(url: string): Promise<ExtractedContent | null> {
  return {
    url,
    title: "YouTube video",
    content:
      "Full YouTube analysis was removed with Gemini/Perplexity support. Use web_search to research the video, or use timestamp/frames with fetch_content to extract still frames.",
    error: "YouTube analysis is not available in this Exa-only setup.",
  };
}
...
export async function extractYouTubeFrames(
  ...
  return { frames, duration: info.duration, error: frames.length === 0 && firstError ? firstError.error : null };
}
